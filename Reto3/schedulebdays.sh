...
echo -e "${SCRIPT_CONTENT}" > "${SCRIPT_DIR}ejecutar_bdays.sh"

# Da permisos de ejecución al script de shell
chmod +x "${SCRIPT_DIR}ejecutar_bdays.sh"

# Configura cron para que ejecute el script de shell cada lunes a la medianoche
(crontab -l 2>/dev/null; echo "0 8 * * * ${SCRIPT_DIR}ejecutar_bdays.sh") | crontab -

echo "Se enviarán las felicitaciones diariamente"
